...
const geoInfo = async (host) => {
  // not IP address
  if (!isIP(host)) {
    return void new Error(`Not IP address: ${host}`);
    //throw new Error(`Not IP address: ${host}`);
  }
  ...
  if (data[0] !== null) {
    ...
    // region
    if (
      Array.isArray(data[0].subdivisions) &&
      data[0].subdivisions.length > 0
    ) {
...
const geoUpdate = async (dbPath, token) => {
  ...
  const promises = [MAXMIND, MAXMIND_ASN, IP2LOCATON, IP2LOCATON_PROXY].map(
    (DB) =>
...
const bgpInfo = async (host, timeout) => {
  ...
  if (net.isIPv4(ip)) {
    ...
    return new Error(`Not valid IPv4: ${ip}`);
  }
